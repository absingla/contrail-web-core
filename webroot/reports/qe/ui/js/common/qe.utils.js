...
define([
    ...
    var QEUtils = function () {
        ...
        self.getToTimeElementConfig = function(fromTimeId) {
            return {
                formatTime: "h:i A",
                format: "M d, Y h:i A",
                displayFormat: "MMM DD, YYYY hh:mm A",
        ...
        self.parseSelectString2Array = function(queryFormModel) {
            ...
            _.forEach(selectFields, function(selectFieldValue) {
        ...
        self.getCheckedFields = function(checkedMap) {
            var checkedFields = [];
            _.forEach(checkedMap, function(checkedMapValue, checkedMapKey){
                if (checkedMapValue()) {
        ...
        self.enableSessionAnalyzer = function(selectedFlowRecord, formModelAttr) {
            var i, enable = true, disable = !enable,
    ...
    function parseLimitBy(limitBy) { // eslint-disable-line
        try {
    ...
    function parseWhereANDClause(whereANDClause) {
        var whereANDArray = whereANDClause.replace("(", "").replace(")", "").split(" AND "),
            whereANDLength = whereANDArray.length, tempWhereANDClauseWithSuffix,
            whereANDClauseArray, operator = "", i;
    ...
    function getOperatorCode(operator) {
        ...
        $.each(cowc.OPERATOR_CODES, function(operatorCodeKey, operatorCodeValue) { // eslint-disable-line
